...
def generate_launch_description():
    # ---- Declare args (strings are fine here; we cast at Node params) ----
    args = [
        # IO topics
    ...
    # ---- YOLO detector node (typed params) ----
    detector = Node(
    ...
    fusion = Node(
        ...
        parameters=[{
            ...
            # fusion behavior (use detector's min_conf; others as fixed typed constants)
            'min_conf': ParameterValue(LaunchConfiguration('det_min_conf'), value_type=float),
            'expand_px': 14,
            'depth_band_beta': 0.6,
            'reuse_last_ms': 600,
